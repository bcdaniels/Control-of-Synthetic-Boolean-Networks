...
def landscape_data(net,pin=None,dynamic_pin=None):
    ...
    the landscape data in the network object.
    """
    ls = net.landscape(pin=pin,dynamic_pin=dynamic_pin)
    data = {'attractors': ls.attractors,
...
def attractors_brute_force(net, size=None, subgraph=None):
    ...
    lsdata = landscape_data(net,pin=pin)
    return lsdata['attractors']
...
def pinning_produces_desired_attractor(net,pin,dynamic_pin_states,desired_attractor):
    ...
    subset_lsdata = landscape_data(net,pin=pin,
                                   dynamic_pin=dynamic_pin_states)
    subset_atts = subset_lsdata['attractors']
    
    if len(subset_atts) == 1:
        # is the single attractor we found the one we want?
        pinned_attractor = subset_atts[0]
...
def module_control_kernel(net,module,input,attractors_given_input,
    require_inputs=True,checkFullCK=True,dynamic=False,verbose=False):
    """
    ...
    checkFullCK (True)      : If True, first check for each attractor whether all nodes are
                              required for successful control.  This is typically much faster
                              if there are attractors for which this is the case.
    ...
    for attractor_index,desired_attractor in enumerate(attractors_given_input):
        ...
        # optionally check first whether all constant nodes are required for control
        if checkFullCK and (ck_list[attractor_index] == -1):
            idx = 0
            uncontrolled = True
            while uncontrolled and idx < len(potential_control_nodes):
                # include all nodes but one
                subset = potential_control_nodes[:idx] + potential_control_nodes[idx+1:]
                # combine dynamic external input and static internal pinning
                # Pin the subset to the first value in the desired attractor.
                pin,dynamic_pin_states = _combine_external_and_internal_pin(
                                            external_pin,input,
                ...
                control_success = pinning_produces_desired_attractor(net,
                                        pin,dynamic_pin_states,desired_attractor_encoded)
                uncontrolled = not control_success
                idx += 1
                # ********** DEBUG *************
                #print("module_control_kernel DEBUG: attractor_index = {}, idx = {}, uncontrolled = {}".format(attractor_index,idx,uncontrolled))
                # ******** END DEBUG ***********
            if uncontrolled and idx == len(potential_control_nodes):
                # if leaving any single node unpinned does not lead to successful control
                # (for all choices of a single node), then the control kernel must consist
                # of all potential nodes
                ck_list[attractor_index] = potential_control_nodes
...
def attractors(net, verbose=False, retall=False, find_control_kernel=False,
    ...
    decode,encode = net.decode, net.encode
    decode_attractors = \
...
def sampled_distinguishing_nodes(net,numsamples=10000,phenotype='all',
                                 seed=123,verbose=False,attractors=None,**kwargs):
    decode,encode = net.decode,net.encode
    ...
    if attractors:
        original_sampled_attractors = attractors
    else:
        original_sampled_attractors = sampled_attractors(net,numsamples=numsamples,
                                                         seed=seed,
                                                         phenotype_list=phenotype_nodes)
    original_sampled_attractors_decoded = \
        [ [ decode(state) for state in att ] for att in original_sampled_attractors ]
    if verbose:
        print("sampled_distinguishing_nodes: number of original attractors:",
              len(original_sampled_attractors))
